...
- For real browser execution, follow that skill and use the global `playwright` skill instead of inventing new one-off browser workflows when an existing QA runner or repo script already fits.
...
- `/home/luke/Desktop/learning/homeschool-v2` is the main checkout on `main`. This is the directory to use when you need the canonical repo root, the default dev server, or the merge target for branch worktrees.
...
- At the start of every session, check whether the main checkout is already serving on `http://localhost:3000`. If it is not running, start it from `/home/luke/Desktop/learning/homeschool-v2` with `corepack pnpm dev` or `make dev` before browser-based validation.
...
- Use the scripted checks that match the change. The minimum gate is `corepack pnpm typecheck`, targeted work should also run the relevant checks such as `corepack pnpm test:architecture`, `corepack pnpm test:curriculum`, `corepack pnpm test:activity`, or `corepack pnpm verify:phase3:rls`, and UI changes should still run `bash ./scripts/verify-before-merge.sh` before merge completion.
...
- Reserve `localhost:3000` for the main checkout at `/home/luke/Desktop/learning/homeschool-v2`. If a worktree session needs browser validation, it must use its own port and report that exact URL, for example `http://localhost:3002/copilot`.
...
- Use `/home/luke/Desktop/learning/homeschool-v2` as the canonical `main` checkout. When work is ready to merge, merge it there. Do not rely on a separate `-main` directory.
...
- Git only allows one worktree to have a branch checked out at a time. If `git switch main` fails because `main` is already checked out at `/home/luke/Desktop/learning/homeschool-v2`, that is expected; use the main checkout directory instead of switching another worktree to `main`.
- Treat `/home/luke/Desktop/learning/homeschool-v2` as the only checkout allowed to own the default localhost session. Any branch worktree that needs a dev server or browser automation must choose a different port so it does not take over the user's main browser view.
...
- Run `corepack pnpm contracts:check` to ensure the registry remains structurally sound before finishing your task.
