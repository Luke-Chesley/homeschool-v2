...
export default async function AssistantPage({ searchParams }: Props) {
  ...
  try {
    ...
    previewError =
      error instanceof Error ? error.message : "Assistant preview is unavailable right now.";
  }

  const suggestedPrompts = [
    "Help me plan today.",
    "What should I move this week?",
    "Explain what comes next in this route.",
          ...
          {previewError ? (
            <Card variant="glass">
              <div className="space-y-2 p-4 text-sm leading-6">
                <p className="font-medium text-foreground">Assistant is temporarily unavailable</p>
